...
#define v_model "models/v_magicknife.mdl"
#define p_model "models/p_magicknife.mdl"
...
{
	"vox/buffclass19s3ct/skill/miracleprismsword1.wav",
	"vox/buffclass19s3ct/skill/miracleprismsword2.wav",
	
	"vox/buffclass19s3tr/skill/miracleprismsword1.wav",
	"vox/buffclass19s3tr/skill/miracleprismsword2.wav"
}
...
new g_had_magicknife[33], g_rb, g_freezetime
new spr_blood_spray, spr_blood_drop

native get_class_mdlname(id, s[], len);
...
public plugin_init() 
{
	...
	g_rb = zp_register_extra_item("Miracle Prism Sword", 30, ZP_TEAM_HUMAN)
...
public zp_extra_item_selected(id, itemid) if(itemid == g_rb) get_magicknife(id)
...
public get_magicknife(id)
{
	...
	mdcsohud_regwpnhud(id, CSW_MAGICKNIFE, spr_wpn)
...
{
	mdcsohud_resetwpnhud(id, CSW_MAGICKNIFE)
...
public AurezLegendOfBotac(id, iEnt, iButton)
{
	...
	if(!(iButton & IN_ATTACK2))
	{
		...
		if(iState == 4)
		{
			...
			// eheh
			new sz[32]; get_class_mdlname(id, sz, 31)
			if(equal(sz, "buffclass19s3ct")) SendSound(id, CHAN_AUTO, chara_specialsound[1])
			if(equal(sz, "buffclass19s3tr")) SendSound(id, CHAN_AUTO, chara_specialsound[3])
		}
	}
	...
	if((iButton & IN_ATTACK2) && !iSnd)
	{
		switch(iState)
		{
			...
			case 3:
			{
				...
				// eheh
				new sz[32]; get_class_mdlname(id, sz, 31)
				if(equal(sz, "buffclass19s3ct")) SendSound(id, CHAN_AUTO, chara_specialsound[0])
				if(equal(sz, "buffclass19s3tr")) SendSound(id, CHAN_AUTO, chara_specialsound[2])
			}
			case 4:
			{
...
public MagicInit(id)
{
	...
	{
		if(!is_user_connected(x) || !is_user_alive(x) || !zp_get_user_zombie(x))
			continue;
...
public MagicThink(Ent)
{
	...
	if(zp_get_user_zombie(pevAttacker) || (get_user_weapon(pevAttacker) != CSW_KNIFE || !g_had_magicknife[pevAttacker]) || !is_user_alive(iVictim) || !is_user_connected(iVictim))
	{
		if(iMode != 2 && iCheck != 2)
